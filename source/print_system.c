...
int sub_printf(u8*);
int prepare_base_10(int, mu8*);
int prepare_base_16(u32, mu8*);
int digits_print(mu8*, int, u8, u8);
...
#endif
mu8 screen_num;
mu8 loaded_screen_num;

mu8 screens_flush;
mu8 enabled_screen[TOTAL_BG];
mu8 updated_screen[TOTAL_BG];
mu8 buffer_screen[TOTAL_BG];
mu8 screen_positions[TOTAL_BG][2];
...
void init_text_system() {
    ...
    // Load japanese font
    //swi_LZ77UnCompWrite8bit(jp_font_c_bin, buffer);
...
int prepare_base_16(u32 number, mu8* digits) {
    for(int i = 0; i < NUM_DIGITS; i++)
        digits[i] = 0;
...
int digits_print(mu8* digits, int max, u8 sub, u8 start_pos) {
