...
IWRAM_CODE void copy_save_to_ram_u16(uintptr_t address, u16* destination, size_t size) {
    // Sanitize size
    ...
    for(size_t n = 0; n < num_banks; n++) {
        ...
        for(size_t i = 0; i < inner_size / 2; i++) {
            destination[i] = read_direct_single_byte_save(address + (i * 2)) | (read_direct_single_byte_save(address + (i * 2) + 1) << 8);
        }
